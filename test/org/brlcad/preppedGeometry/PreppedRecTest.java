 ...
 * @author jra
 */
public class PreppedRecTest {
        double tol = 0.005;

    public PreppedRecTest() {
    }
    ...
    public void testPrep() {
        try {
            ...
            Vector3 c = new Vector3(100.0, 0.0, 0.0);
            Vector3 d = new Vector3(0.0, 50.0, 0.0);
            ...
            PreppedObject preppedRec = tgc.prep(reg, preppedDb, matrix);
            BoundingBox expectedBB = new BoundingBox(new Point(-100, -50, 0), new Point(100, 50, 1000));
            Point expectedCenter = new Point(0, 0, 500);
            double expectedBR = 500.0;
            assertTrue("BoundingBox should be " + expectedBB + ", but was " +
                    preppedRec.getBoundingBox(), expectedBB.equals(preppedRec.getBoundingBox()));
            assertTrue("Bounding radius should be " + expectedBR + ", but was " +
                    preppedRec.getBoundingRadius(), expectedBR == preppedRec.getBoundingRadius());
            assertTrue("Center should be " + expectedCenter + ", but was " +
                    preppedRec.getCenter(), expectedCenter.equals(preppedRec.getCenter()));
        } catch (Exception ex) {
            fail(ex.getMessage());
        }
    }
    ...
    public void testShoot() {
        try {
            ...
            Point expectedEntrance = new Point(-100, 0, 500);
            double expectedEntranceDist = 900;
            Vector3 expectedEntranceNorm = new Vector3(-1, 0, 0);
            int expectedEntranceSurfno = Tgc.BODY;
            Point expectedExit = new Point(100, 0, 500);
            double expectedExitDist = 1100;
            Vector3 expectedExitNorm = new Vector3(1, 0, 0);
    ...
    public void testShootwMatrix() {
        try {
            ...
            Point expectedEntrance = new Point(1262.87, 3456, 4817);
            expectedEntrance.setTolerance(tol);
            double expectedEntranceDist = 2262.87;
            Vector3 expectedEntranceNorm = new Vector3(-0.866025, 0, 0.5);
            expectedEntranceNorm.setTolerance(tol);
            int expectedEntranceSurfno = Tgc.BODY;
            Point expectedExit = new Point(1493.81, 3456, 4817);
            expectedExit.setTolerance(tol);
            double expectedExitDist = 2493.81;
            Vector3 expectedExitNorm = new Vector3(0.866025, 0, -0.5);
            expectedExitNorm.setTolerance(tol);
            int expectedExitSurfno = Tgc.BODY;
            assertTrue("expected entrance hit at " + expectedEntrance + ", but got " +
                    seg.getInHit().getHit_pt(), expectedEntrance.equals(seg.getInHit().getHit_pt()));
    ...
    public void testMakeSegs() {
        try {
            ...
            List<Segment> segs = preppedRec.makeSegs(hits, ray, rayData);
